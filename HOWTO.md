...
document, we assume that user is called 'xGOx'). We also assume the
xGOx user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run xGOxd and keep a copy of the full blockchain,
which is roughly 125 GB (January 2017). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit xGOxd 
...
only leaving spare cycles for xGOxd. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running xGOxd and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `xGOxd` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser xGOx --disabled-password
    $ sudo apt-get install git
    $ sudo su - xGOx
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/xGOx/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download xGOxd

We currently recommend xGOx core 0.15.0 stable. If your package manager does not supply
a recent xGOxd or you prefer to compile it yourself, here are some pointers for Ubuntu:

    $ sudo apt-get install make bsdmainutils g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - xGOx
    $ cd ~/src && wget https://xGOx.org/bin/xGOx-core-0.15.0/xGOx-0.15.0.tar.gz
    $ sha256sum xGOx-0.15.0.tar.gz | grep 54b6f54982da97f294d21ad69c6b8624f2cf40d157be0683123b2ba6db2bf2a1
    $ tar xfz xGOx-0.15.0.tar.gz
    $ cd xGOx-0.15.0
    $ ./configure --disable-wallet --without-miniupnpc
    $ make
    $ strip src/xGOxd src/xGOx-cli src/xGOx-tx
    $ cp -a src/xGOxd src/xGOx-cli src/xGOx-tx ~/bin

### Step 3. Configure and start xGOxd

In order to allow Electrum to "talk" to `xGOxd`, we need to set up an RPC
username and password for `xGOxd`. We will then start `xGOxd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.xGOx
    $ $EDITOR ~/.xGOx/xGOx.conf

Write this in `xGOx.conf`:

    daemon=1
    txindex=1

rpcuser / rpcpassword options are only needed for non-localhost connections.
you can consider setting maxconnections if you want to reduce xGOxd bandwidth
(as stated above)

If you have an existing installation of xGOxd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ xGOxd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `xGOxd`:

    $ xGOxd

Allow some time to pass for `xGOxd` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ xGOx-cli getblockchaininfo

Before starting the Electrum server your xGOxd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start xGOxd at boot
time, running as the 'xGOx' user. Check your system documentation to
...
file includes the database setup, xGOxd RPC setup, and a few other
options.
...
If you're using the user `xGOx` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "xGOx hard nofile 131072" >> /etc/security/limits.conf
     echo "xGOx soft nofile 131072" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u xGOx -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close xGOxd for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting xGOxd (together with electrum-server) on a weekly basis to clear out unconfirmed
...
xGOxs to confirm that everything is working properly.
...
https://xGOxtalk.org/index.php?topic=85475.0
