...
document, we assume that user is called 'GoByte'). We also assume the
GoByte user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run GoByted and keep a copy of the full blockchain,
which is roughly 125 GB (January 2017). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit GoByted 
...
only leaving spare cycles for GoByted. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running GoByted and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `GoByted` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser GoByte --disabled-password
    $ sudo apt-get install git
    $ sudo su - GoByte
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/GoByte/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download GoByted

We currently recommend GoByte core 0.12.13 stable. If your package manager does not supply
a recent GoByted or you prefer to compile it yourself, here are some pointers for Ubuntu:
    ...
    $ make
    $ strip src/gobyted src/gobyte-cli src/gobyte-tx
    $ cp -a src/gobyted src/gobyte-cli src/gobyte-tx ~/bin

### Step 3. Configure and start GoByted

In order to allow Electrum to "talk" to `GoByted`, we need to set up an RPC
username and password for `GoByted`. We will then start `GoByted` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.gobytecore
    $ $EDITOR ~/.gobytecore/gobyte.conf

Write this in `GoByte.conf`:

    daemon=1
    txindex=1

rpcuser / rpcpassword options are only needed for non-localhost connections.
you can consider setting maxconnections if you want to reduce GoByted bandwidth
(as stated above)

If you have an existing installation of GoByted and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ gobyted -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `GoByted`:

    $ gobyted

Allow some time to pass for `GoByted` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ gobyte-cli getblockchaininfo

Before starting the Electrum server your GoByted should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start GoByted at boot
time, running as the 'GoByte' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/Nillecram/electrum-server.git
    $ cd electrum-server
...
package manager if you don't want to use the install routine.
    ...
    $ sudo apt-get install python-setuptools python-openssl python-leveldb libleveldb-dev python-dev libxml2-dev libxslt-dev python-scrypt python-pip
    $ sudo easy_install jsonrpclib irc plyvel
    $ pip install coinhash
...
file includes the database setup, GoByted RPC setup, and a few other
options.
...
If you're using the user `GoByte` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "GoByte hard nofile 131072" >> /etc/security/limits.conf
     echo "GoByte soft nofile 131072" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u GoByte -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close GoByted for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting GoByted (together with electrum-server) on a weekly basis to clear out unconfirmed
...
GoBytes to confirm that everything is working properly.
...
https://GoBytetalk.org/index.php?topic=85475.0
